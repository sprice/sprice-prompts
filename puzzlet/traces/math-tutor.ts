...
async function run () {
  try {
    ...
    console.log('Fetching prompt "math-tutor.prompt.mdx"...');





    const basicPrompt = await puzzletClient.fetchPrompt("math-tutor.prompt.mdx");
    console.log('Successfully fetched prompt');
    
    const props = { userMessage: 'What is the square root of 9?' };
    const telemetry = {
      isEnabled: true,
